...
const CardMoviesComponents = ({data, mediaType}) =>{
    ...
    const title = data.title || data.name; //data.original_language(around 1 hour 50 min) in youtube I changed cause it comes in chinese.

    const vote_average = parseInt(data.vote_average);//to show without decimal, if removed parseInt shows in decimal
   const original_language = data.original_language || ''; //here ? can be used if it's there then show that else show others purposes. Watch video. He explains it like this.
   //doesn't work const original_language = data.title ? data.title : data.original_title;
